...
export const useMediaPipe = ({
  ...
  useEffect(() => {
    let isActive = true; // 👈 flag de vida útil
    ...
    const initializeMediaPipe = async () => {
      try {
        ...
        const hands = new window.Hands({
          locateFile: (file) =>
            `https://cdn.jsdelivr.net/npm/@mediapipe/hands/${file}`, // ✅ backticks
        });
        ...
        hands.onResults((res) => {
          if (!isActive) return; // 👈 evita usar después del cleanup
          onResults(res);
        });
    ...
    return () => {
      isActive = false; // 👈 corta el flujo de frames
